...
from opentinker.backend_patch.verl.trainer.ppo.reward import compute_reward, compute_reward_async, load_reward_manager
...
def build_role_worker_mapping(config):
    ...
    if need_critic(config):
        ...
        print(f"✓ Critic worker added to role_worker_mapping")
    else:
    ...
    if hasattr(config, "algorithm") and (
        ...
        if config.actor_rollout_ref.actor.strategy in {"fsdp", "fsdp2"}:
            ...
            raise NotImplementedError(f"Unsupported ref policy strategy: {config.actor_rollout_ref.actor.strategy}")

        role_worker_mapping[Role.RefPolicy] = ray.remote(ActorRolloutRefWorker)
        print(f"✓ RefPolicy worker added to role_worker_mapping")
    ...
    if config.reward_model.enable:
        ...
        print(f"✓ RewardModel worker added to role_worker_mapping")
...
def build_reward_fn(config, tokenizer):
    ...
    is_gomoku = False
    try:
        interaction_config_path = config.actor_rollout_ref.rollout.multi_turn.get("interaction_config_path")
    ...
    if is_gomoku:
        try:
            ...
            print("[build_reward_fn] Detected Gomoku environment, wrapping with GomokuRewardManager for metrics")
            reward_fn = GomokuRewardManager(tokenizer=tokenizer, base_reward_fn=reward_fn, num_examine=0)
            val_reward_fn = GomokuRewardManager(tokenizer=tokenizer, base_reward_fn=val_reward_fn, num_examine=1)
        except ImportError as e:
            logger.warning(f"Failed to import GomokuRewardManager: {e}. Using standard reward function.")
...
class PPOTrainingServerBackend:
    ...
    def __init__(
        ...
        self.generation_config = {
            ...
            "max_new_tokens": config.actor_rollout_ref.rollout.get("max_new_tokens", 4096),
        }
    ...
    def init_workers(self, total_steps: int) -> Dict[str, Any]:
        ...
        try:
            ...
            self.ref_policy_wg = self.trainer.ref_policy_wg if self.use_reference_policy and not self.ref_in_actor else None
    ...
    def compute_rollout_importance_weights_and_add_to_batch(self, batch: DataProto) -> tuple[DataProto, dict]:
        ...
        if rollout_is_threshold is not None and rollout_is_threshold > 0 and "rollout_log_probs" in batch.batch:
            logger.info(f"DEBUG: Computing rollout IS weights. Batch keys: {batch.batch.keys()}")
    ...
    def train_step(self, batch: DataProto) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"DEBUG: batch keys after gen union: {list(batch.batch.keys())}")
            ...
            with marked_timer("old_log_prob", timing_raw, color="blue"):
                # ===== DEBUG LOGGING START =====
                logger.info("=" * 80)
                logger.info(f"[OPENTINKER] Batch state before compute_log_prob (step {self.global_steps})")
                ...
                logger.info(f"  attention_mask shape: {batch.batch['attention_mask'].shape}")
                logger.info(f"  response_mask shape: {batch.batch['response_mask'].shape}")
                logger.info(f"  response_mask sum: {batch.batch['response_mask'].sum().item()}")
                logger.info(f"  response_mask mean: {batch.batch['response_mask'].float().mean().item()}")
                logger.info(f"  First response (first 20 tokens): {batch.batch['responses'][0, :20].tolist()}")
                logger.info(f"  First response_mask (first 20): {batch.batch['response_mask'][0, :20].tolist()}")
                logger.info(f"  batch.meta_info: {batch.meta_info}")
                logger.info("=" * 80)
                # ===== DEBUG LOGGING END =====
                
                old_log_prob = self.actor_rollout_wg.compute_log_prob(batch)
                logger.info(f"DEBUG: old_log_prob keys: {list(old_log_prob.batch.keys())}")
                ...
                # ===== DEBUG LOGGING START =====
                logger.info(f"[OPENTINKER] After compute_log_prob (step {self.global_steps})")
                ...
                logger.info(f"  First entropy values (first 20): {entropys[0, :20].tolist()}")
                logger.info("=" * 80)
                # ===== DEBUG LOGGING END =====
                ...
                logger.info(f"DEBUG: batch keys after old_log_prob union: {list(batch.batch.keys())}")
                ...
                for key, values in reward_extra_infos_dict.items():
                    ...
                    try:
                        ...
                        if np.issubdtype(flat_values.dtype, np.number):
                            ...
                            if key in ["win_rate", "loss_rate", "draw_rate", "wins", "losses", 
                                      "draws", "invalid_games", "valid_game_rate", "avg_moves_per_game", 
                                      "total_games"]:
    ...
    def _prepare_generation_batch(self, batch: DataProto) -> DataProto:
        ...
        logger.info(f"DEBUG _prepare_generation_batch: batch.non_tensor_batch.keys() = {list(batch.non_tensor_batch.keys())}")
        
        # Extract logic from RayPPOTrainer._get_gen_batch
        reward_model_keys = set({"data_source", "reward_model", "extra_info", "uid"}) & batch.non_tensor_batch.keys()
        
        logger.info(f"DEBUG _prepare_generation_batch: reward_model_keys = {reward_model_keys}")
        ...
        logger.info(f"DEBUG _prepare_generation_batch: gen_batch.non_tensor_batch.keys() = {list(gen_batch.non_tensor_batch.keys())}")
    ...
    def _prepare_val_batch(self, batch: DataProto) -> DataProto:
        ...
        gen_batch.meta_info["do_sample"] = self.config.actor_rollout_ref.rollout.val_kwargs.do_sample
    ...
    def _generate_sequences(self, gen_batch: DataProto) -> DataProto:
        ...
        gen_batch_padded, pad_size = pad_dataproto_to_divisor(gen_batch_repeated, size_divisor)
    ...
    def validate_step(self, batch: DataProto) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(min(10, len(inputs))):  # Return top 10 samples
                samples.append({"input": inputs[i], "output": outputs[i], "score": float(scores[i])})
...
async def init_workers(request: InitWorkersRequest):
    ...
    if _training_server is None:
        logger.info("Server backend not yet initialized. Waiting for initialization to complete...")
...
async def override_config(request: ConfigOverrideRequest):
    ...
    try:
        ...
        logger.info(f"[DEBUG] BEFORE merge - tokenizer path: {_server_cfg.actor_rollout_ref.model.path}")
        logger.info(f"[DEBUG] Client config overrides: {OmegaConf.to_yaml(OmegaConf.create(config_overrides))}")
        
        logger.info(f"Merging configuration overrides")
        ...
        logger.info(f"[DEBUG] AFTER merge - tokenizer path: {_server_cfg.actor_rollout_ref.model.path}")
        ...
        try:
            ...
            interaction_config_content = multi_turn_cfg.get("interaction_config_content", None)
            if interaction_config_content:
                import tempfile
                import yaml
                
                # Create a new local temp file with the content
                fd, local_path = tempfile.mkstemp(suffix='.yaml', prefix='interaction_config_server_')
                ...
                logger.info(f"[CROSS-NODE] Created local interaction config from content: {local_path}")
        except Exception as e:
...
async def train_step(request: TrainStepRequest):
    ...
    try:
        ...
        result = await loop.run_in_executor(_train_executor, _training_server.train_step, batch)
...
async def validate(request: ValidateRequest):
    ...
    try:
        ...
        loop = asyncio.get_running_loop()
        result = await loop.run_in_executor(_train_executor, _training_server.validate_step, val_batch)
...
async def upload_reward_function(request: UploadRewardFunctionRequest):
    ...
    try:
        ...
        if _server_cfg is not None:
            ...
            logger.info(f"Updated server config: custom_reward_function.path={temp_file}, name={request.function_name}")
...
def launch_server(
    ...
    print(f"\n{'=' * 60}")
    print(f"Starting HTTP PPO Training Server (Delayed Initialization Mode)")
    ...
    print(f"  - Override config (CALL THIS FIRST): http://{cfg.server.host}:{cfg.server.port}/api/v1/override_config")
    print(f"  - Init workers: http://{cfg.server.host}:{cfg.server.port}/api/v1/init_workers")
    print(f"  - Train step: http://{cfg.server.host}:{cfg.server.port}/api/v1/train_step")
    print(f"\nClient workflow:")
    print(f"  1. Send config overrides to /api/v1/override_config")
    print(f"  2. Call /api/v1/init_workers")
    print(f"  3. Send training batches to /api/v1/train_step")
    print(f"\n{'=' * 60}\n")
    ...
    try:
        ...
        print(f"Initializing Ray (address={_server_cfg.ray.address}, namespace={_server_cfg.ray.namespace})")
        
        # Check if Ray is already initialized in this process
        if ray.is_initialized():
            print("⚠️  Ray is already initialized in this process, shutting it down first...")
            ray.shutdown()
        ...
        if _server_cfg.ray.address == "auto":
            ...
            print(f"No existing Ray cluster found, starting new one with {_server_cfg.trainer.n_gpus_per_node} GPUs...")
            ray.init(
                namespace=_server_cfg.ray.namespace,
        ...
        print(f"✓ Ray initialized successfully in main thread")
        print(f"  - Available GPUs: {available_gpus}")
        print(f"  - Required GPUs: {_server_cfg.trainer.n_gpus_per_node}")
        
        if available_gpus < _server_cfg.trainer.n_gpus_per_node:
            print(f"\n{'='*60}")
            print(f"⚠️  WARNING: GPU Mismatch!")
            print(f"⚠️  Available GPUs ({available_gpus}) < Required GPUs ({_server_cfg.trainer.n_gpus_per_node})")
            print(f"⚠️  ")
            print(f"⚠️  This will cause worker initialization to fail!")
            print(f"⚠️  ")
            print(f"⚠️  Solution:")
            print(f"⚠️  1. Stop all Ray instances: ray stop --force")
            print(f"⚠️  2. Start Ray with GPUs: ray start --head --num-gpus={_server_cfg.trainer.n_gpus_per_node}")
            print(f"⚠️  3. Re-run this script")
            print(f"{'='*60}\n")
            raise RuntimeError(f"Insufficient GPUs: {available_gpus} < {_server_cfg.trainer.n_gpus_per_node}")
        ...
        if _server_cfg.enable_agent_loop:
            ...
            try:
                ...
                tool_config_path = _server_cfg.tool_config_path or "tool/tool_config.json"
                tool_config = {
                    "tools": [
                        {
                ...
                logger.info(f"✓ Tool config path set in config")
                logger.info("="*60)
            except Exception as e:
                logger.error(f"Failed to start Sandbox server: {e}")
                logger.error("Make sure sandbox.py and sandbox_tool.py are in examples/tutorial/agent_loop_get_started/")
                import traceback
                traceback.print_exc()
                raise
        ...
        tokenizer = AutoTokenizer.from_pretrained(_server_cfg.actor_rollout_ref.model.path)
        ...
        logger.info(f"Server is now ready to accept init_workers and train_step requests")
        ...
        logger.info("Main thread waiting for server (press Ctrl+C to shutdown gracefully)...")
        try:
